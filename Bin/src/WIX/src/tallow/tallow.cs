	...
	public class Tallow
	{
		...
		internal class TallowMain
		{
			...
			public TallowMain(string[] args)
			{
				...
				// see if there is a tallow.errors.config file which will have XML config info on
				// how to handle exceptions:
				XmlDocument ErrorConfig = new XmlDocument();
				try
				{
					ErrorConfig.Load("tallow.errors.config");
				}
				catch
				{
					// If the file can't be loaded or is corrupt, we will just do default behavior.
					ErrorConfig = null;
				}
				...
				if (0 < this.comregAssemblies.Count || 0 < this.dirwalkDirectories.Count || 0 < this.selfregFiles.Count || 0 < this.selfRegExes.Count || 0 < this.selfregTlbs.Count)
				{
					...
					foreach (string assembly in this.comregAssemblies)
					{
						...
						try
						{
							...
							{
								// In this case, the main assembly was found, but one of its dependencies was not.
								// This is usually caused when some developer put the main assembly somewhere
								// else (like DistFiles) which does not reflect what the end user will get.
								// We will allow the build to survive, but send an email out to warn key developers
								// about the problem:
								if (!this.oneResourcePerComponent)
									this.EndComponentElement(writer); // </Component>

								if (ErrorConfig != null)
									HandleReportableException("GetExportedTypes_FileNotFoundException", e.ToString(), assembly, ErrorConfig, writer);
								else
									Console.Error.Write("Failed to load Assembly: {0}\r\n {1}", assembly, e.ToString());

								continue; // Don't attempt registry of assembly
							}
						...
						{
							if (ErrorConfig != null)
								HandleReportableException("FileNotFoundException", e.ToString(), assembly, ErrorConfig, writer);
							else
							{
								Console.Error.Write("Failed to load Assembly: {0}\r\n {1}", assembly, e.ToString());
						...
						{
							if (ErrorConfig != null)
								HandleReportableException("BadImageFormatException", e.ToString(), assembly, ErrorConfig, writer);
							else
								Console.Error.Write("Failed to load Assembly: {0}\r\n {1}", assembly, e.ToString());
						}
						...
						{
							if (ErrorConfig != null)
								HandleReportableException("SecurityException", e.ToString(), assembly, ErrorConfig, writer);
							else
							{
								Console.Error.Write("Failed to load Assembly: {0}\r\n {1}", assembly, e.ToString());
								writer.WriteStartElement("Error");
						...
						{
							if (ErrorConfig != null)
								HandleReportableException("PathTooLongException", e.ToString(), assembly, ErrorConfig, writer);
							else
							{
								Console.Error.Write("Failed to load Assembly: {0}\r\n {1}", assembly, e.ToString());
								writer.WriteStartElement("Error");
			...
			/// <summary>
			/// Sends an email to a specified user warning them of a Tallow error
			/// </summary>
			/// <param name="Recipient">Email address to send it to.</param>
			/// <param name="Message">Details of Tallow error.</param>
			private void SendErrorEmail(String Recipient, String Message)
			{
				const String MailServer = "mail.jaars.org";
				const String MailSender = "alistair_imrie@sil.org";
				const String MailSubject = "Automatic message from Tallow (part of installer build)";

				String Email = MailServer + " " + MailSender + " " + Recipient + " \"" + MailSubject + "\" \"" + Message + "\"";
				Process.Start("DosMail.exe", Email);
			}

			private void HandleReportableException(String ExceptionType, String ExceptionDesc, String AssemblyPath,
				XmlDocument ErrorConfig, XmlTextWriter writer)
			{
				XmlNode HandlerNode = ErrorConfig.SelectSingleNode("//Handler[@error='" + ExceptionType + "']");
				if (HandlerNode != null)
				{
					XmlNode ConsoleReportNode = HandlerNode.SelectSingleNode("ConsoleReport");
					XmlNode XmlReportNode = HandlerNode.SelectSingleNode("XmlReport");
					XmlNodeList EmailNodes = HandlerNode.SelectNodes("Email");

					if (ConsoleReportNode != null)
					{
						if (ConsoleReportNode.InnerText == "true")
							Console.Error.Write("Failed to load Assembly: {0}\r\n {1}", AssemblyPath, ExceptionDesc);
					}

					if (XmlReportNode != null)
					{
						if (XmlReportNode.InnerText == "true")
						{
							writer.WriteStartElement("Error");
							writer.WriteAttributeString("Tallow", ExceptionType);
							writer.WriteEndElement();
						}
					}
					...
					if (EmailNodes != null)
					{
						String MailMessage = "Assembly " + AssemblyPath + " could not be registered\n\n";// + ExceptionDesc;

						foreach (XmlNode EmailAddress in EmailNodes)
						{
							String EmailRecipient = EmailAddress.InnerText;
							SendErrorEmail(EmailRecipient, MailMessage);
						}
					}
				}
			}
