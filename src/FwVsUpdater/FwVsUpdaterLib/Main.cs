		...
		public void MainMethod(string[] args)
		{
			...
			for (int iArgs = 0; iArgs < args.Length; iArgs++)
			{
				...
				else
				{
					MessageBox.Show(
						...
						"{0}. (C) 2008-2011 by SIL International", AppName));
					Trace.WriteLine(DateTime.Now + ": Displayed usage. Exiting");
					return;
				}
			}
			...
			if (Properties.Settings.Default.FwRoot.Length == 0)
			{
				using (FwPath fwPath = new FwPath())
				{
					...
					{
						Trace.WriteLine(DateTime.Now + ": Can't continue without a FW path");
						return;
					}
				}
			}
